...
public class AdaptNormals1 extends PApplet {
	...
	private void evo(){
		...
		for (int i = 0; i < membsize; i++) {
			...
			double ang = 0.5 * Math.acos(cos);
			int sign = M.dotProduct(c0, dir) < 0 ? 1 : -1;
			double[] nv0 = M.rotate_Rodriguez(candis[m0], sign * ang, d);
			double[] nv1 = M.rotate_Rodriguez(candis[m1], -sign * ang, d);
			M.assign(nors[m0], M.normalize(nv0));
			M.assign(nors[m1], M.normalize(nv1));
		}
	}
