...
public class AdaptNormals2c extends PApplet {
	...
	private final double evo_learnrate = 0.15;
	private final double diff_normal=20* Math.PI/180 ;   //30 is almost free
	private double[] bar_vars;
	...
	public void setup() {
		...
		bar_vars=new double[mesh.membsize];
		for (int i = 0; i < 200; i++)
			evo();
		...
		for (int i = 0; i <mesh.membsize; i++) {
			println( df.format(bar_vars[i]* 180 / PI));
		}
	}
	...
	private void evo() {
		...
		for (int i = 0; i < nodesize; i++) {
			...
			double ang = Math.acos(M.dotProduct(result, nors[i]));
			if (ang > diff_normal) {
				double[] axis = M.crossProduct(result, nors[i]);
				result = M.rotate_Rodriguez(result,  1* (ang - diff_normal), axis);
			}
