...
public class TestKuka_for extends PApplet {
	...
	private double[][] for_cylinder = new double[4][]; // left
	private double[][] for_hex = new double[6][]; // right
	private final double cylinder_thick = 7; // phi8
	private final double hex_thick = 7;
	private final double cylinder_radius = 0.2 + 0.5 * 13 - 0.5 * 3;// +0.2
	private final double hex_radius = 0.2 + 0.5 * 15 - 0.5 * 3;// +0.2
	...
	public void setup() {
		...
		global_shift = new double[] { base_shift, -Pavilion.hei / 2, -Pavilion.wid / 2 };// x****

		tim.toolPath(toolR, margin);
		writeKRL("/Users/huahao/Desktop/For" + tid + "_Jun28.src");
	}
	...
	private void forDetial() {
		double[] cnt = { tim._forhole[0], 0.5 * Pavilion.hei - cylinder_thick, tim._forhole[2] }; //****for differs back
		for_cylinder[0] = M.add(cnt, new double[] { 0, 0, cylinder_radius });
		for_cylinder[1] = M.add(cnt, new double[] { cylinder_radius, 0, 0 });
		for_cylinder[2] = M.add(cnt, new double[] { 0, 0, -cylinder_radius });
		for_cylinder[3] = M.add(cnt, new double[] { -cylinder_radius, 0, 0 });

		cnt = new double[] { tim._forhole[0], -0.5 * Pavilion.hei + hex_thick, tim._forhole[2] }; //****for differs back
		for (int i = 0; i < 6; i++) {
			double a = i * 2 * PI / 6.0;
			double[] v = { hex_radius * Math.sin(a), 0, hex_radius * Math.cos(a) };
			for_hex[i] = M.add(cnt, v);
		}
		...
		double[] abc = LA.ABCby2Axis(Timber.downy, Timber.downz); //****for differs back
		double[] readyp = M.add(for_cylinder[0], new double[] { 0, 20, 0 }); //****for differs back
		wt.LIN_CDIS(M.add(readyp, global_shift), abc);
		wt.VEL(mid);
		wt.LIN_CDIS(M.add(for_cylinder[0], global_shift), abc);
		wt.VEL(slow);
		wt.CIRC_CDIS(M.add(for_cylinder[1], global_shift), M.add(for_cylinder[2], global_shift));
		wt.CIRC_CDIS(M.add(for_cylinder[3], global_shift), M.add(for_cylinder[0], global_shift));
		wt.VEL(fast);
		wt.LIN_CDIS(M.add(readyp, global_shift), abc);
		...
		abc = LA.ABCby2Axis(Timber.upy, Timber.upz);  // //****for differs back
		readyp = M.add(for_hex[0], new double[] { 0, -20, 0 }); //****for differs back
		wt.LIN_CDIS(M.add(readyp, global_shift), abc);
		wt.VEL(mid);
		wt.LIN_CDIS(M.add(for_hex[0], global_shift), abc);
		wt.VEL(slow);
		for (int i = 1; i < for_hex.length; i++)
			wt.LIN_CDIS(M.add(for_hex[i], global_shift), abc);
		wt.LIN_CDIS(M.add(for_hex[0], global_shift), abc);
		wt.VEL(fast);
		wt.LIN_CDIS(M.add(readyp, global_shift), abc);
	...
	private void forHole() {
		...
		double[] abc, p;
		if (2 > mesh.fixed[tim.m0]) {
			abc = LA.ABCby2Axis(Timber.downy, Timber.downz); // /********for differs back
			p = new double[] { tim._forhole[0], (0.5 * Pavilion.hei + 10), tim._forhole[2] }; // /********for differs back
		} else {
			double[] holeOnFace = tim._uphole[1] > 0 ? tim._uphole : tim._dnhole;
			if (holeOnFace[1] < 0)
				throw new RuntimeException();
			double[] dir = M.sub(holeOnFace, tim._forhole);// outward
		...
		wt.VEL(2*slow);
		wt.LIN_CDIS(M.add(tim._forhole, global_shift), abc);
		wt.VEL(fast/2);
		wt.LIN_CDIS(M.add(p, global_shift), abc);
	...
	private void forCorner() {
		...
		double[] abc = tim._forleftABC;// ******
		double[][] curve;
		if (0 < mesh.fixed[tim.m0])
			curve = new double[][] { tim._forts_hale[0], tim._forts_hale[3] };
		else
			curve = tim._forts_hale; // ***half left tim._forts_hale
		double[] p = curve[0];
		wt.LIN_CDIS(M.add(p, global_shift), abc);
		wt.VEL(slow);
		...
		if (0 < mesh.fixed[tim.m0])
			curve = new double[][] { tim._forts[0], tim._forts[3] };
		else
			curve = tim._forts;
		...
		abc = tim._forrightABC; // ********
		if (0 < mesh.fixed[tim.m0])
			curve = new double[][] { tim._forts_hari[0], tim._forts_hari[3] };
		else
			curve = tim._forts_hari; // _forts_hari
		p = curve[0];
		wt.LIN_CDIS(M.add(p, global_shift), abc);// ******
		wt.VEL(slow);
		for (int i = 1; i < curve.length; i++) {
